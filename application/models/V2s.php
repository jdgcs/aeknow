...
class V2s extends CI_Model {
	...
	public function getTxByHeight($ak,$startheight,$endheight){
		$this->load->database();
		$trans_sql="SELECT txhash,txtype,sender_id,tx,block_height,recipient_id FROM txs WHERE (sender_id='$ak' OR  recipient_id='$ak') AND block_height>$startheight  AND block_height<$endheight ORDER BY block_height desc,tid desc";		

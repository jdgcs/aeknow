		...
		foreach ($query->result() as $row){
			if(trim($row->contract)!=""){
				...
				if($caller==$owner_id){//the token owned list first
					...
					$ownerlists.="<tr><td>".$row->alias."</td><td>".$row->decimal."</td><td>".number_format($row->balance/pow(10,$row->decimal),0,'','')."</td><td align=center><div class=btn-group><a href=/viewtoken?contractid=".$row->contract."><button type=\"button\" class=\"btn btn-success\">Transfer</button></a> &nbsp;</div> $ownerfunc</td></tr>";
			
					}else{
						$otherlists.="<tr><td>".$row->alias."</td><td>".$row->decimal."</td><td>".number_format($row->balance/pow(10,$row->decimal),0,'','')."</td><td align=center><div class=btn-group><a href=/viewtoken?contractid=".$row->contract."><button type=\"button\" class=\"btn btn-success\">Transfer</button></a> &nbsp;</div></td></tr>";
					}
			}
			}

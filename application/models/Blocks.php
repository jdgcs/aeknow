				...
				if($key=='tx'){
					...
					foreach ($content as $key_tx=>$content_tx){			
						...
						if($key_tx=="payload"){							
							...
							$content_tx=$content_tx."<br />(Decoded:<br/><textarea rows=3 style=\"width:100%;\">".$fordecode."</textarea>)";
							}
						...
						$data['table_data'].='<tr><td><b>'.$key_tx.'</b> </td><td  style="word-wrap:break-word;word-break:break-all;" width="100px";>'.$content_tx.'</td></tr>';
						}
					}else{					
					if($key=='signatures'){
						//echo $key,': ',$content[0],"\n";

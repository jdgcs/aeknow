...
public function getOracleList(){
	...
	$sql="with t as (select distinct(sender_id),max(block_height) as block_height from tx where block_height >".($topheight-10000)." AND (txtype='OracleExtendTx' OR txtype='OracleRegisterTx' OR txtype='OracleRespondTx') group by sender_id) select sender_id,block_height from t order by t.block_height desc;";
	$query = $this->db->query($sql);
